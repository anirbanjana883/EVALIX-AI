  ...
  async evaluateBackground(submissionId, assignment, studentAnswers) {
    ...
    for (const answer of studentAnswers) {
      try {
        ...
        // 🌟 FIX: Added dbQuestion.question_text as the first parameter!
        const aiResult = await llmAgent(
          dbQuestion.question_text, // The actual question asked
          processedText,            // The student's OCR'd answer
          dbQuestion.model_answer,  // The teacher's rubric
          dbQuestion.max_marks      // Maximum points possible
        );
